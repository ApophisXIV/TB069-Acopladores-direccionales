...
#let basic-report(
  ...
  compact-mode: false,
  heading-color: blue,
  heading-font: "libertinus serif", // recommended alternatives: "Fira Sans", "Lato", "Source Sans Pro"
  abstract: none,
  body,
) = {
  ...
  show outline.entry.where(level: 2).or(outline.entry.where(level: 3)).or(outline.entry.where(level:4)): it => {
  ...
  if (show-outline and not compact-mode) {
    ...
    counter(page).update(2)     // so the first chapter starts at page 1 (now in arabic numbers)
  } else {
    ...
    header: context {
      if compact-mode and (counter(page).get().first() == 1) {
        ...
        grid(
          ...
          align: (left, right),
          row-gutter: 0.5em,
          // El titulo del encabezado
          text(font: heading-font, size: label-size,hydra(1,use-last:false,book:false,)),
